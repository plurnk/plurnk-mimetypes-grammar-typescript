#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-typescript.
...
await using temporary = await mkdtempDisposable(path.join(tmpdir(), "grammar-typescript-build-"));
...
// Locate produced wasm and copy as typescript.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "typescript.wasm"));
const bytes = (await readFile(path.join(repoRoot, "typescript.wasm"))).length;
console.log(`typescript.wasm: ${bytes} bytes (built from ${pin})`);
